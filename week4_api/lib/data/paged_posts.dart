...
class PagedPostsState {
  const PagedPostsState({this.items = const [], this.page = 0, this.isLoadingMore = false, this.hasMore = true, this.error});
...
class PagedPostsNotifier extends Notifier<PagedPostsState> {
  ...
  Future<void> loadFirstPage() async {
    ...
    try {
      final items = await repo.fetchPostsPage(page: 1, limit: 10);
  ...
  Future<void> loadNextPage() async {
    ...
    final repo = ref.read(postRepositoryProvider);
    state = PagedPostsState(items: state.items, page: state.page, isLoadingMore: true, hasMore: state.hasMore);
    try {
      final next = state.page + 1;
      final items = await repo.fetchPostsPage(page: next, limit: 10);
      state = PagedPostsState(items: [...state.items, ...items], page: next, hasMore: items.length == 10);
    } catch (e) {
      state = PagedPostsState(items: state.items, page: state.page, error: e);
    }
  }
}
